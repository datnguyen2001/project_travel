...
@section('main')
    <main id="main" class="main">
        ...
        <div class="pagetitle d-flex align-items-center justify-content-between">
            <h1>Danh sách</h1>
            <a href="{{route('admin.tourist.articles.create')}}" class="btn btn-info text-white">Thêm mới</a>
        </div><!-- End Page Title -->
                ...
                @if(count($listData))
                    @foreach($listData as $key => $value)
                        <tr>
                            ...
                            <td>
                                <div class="d-flex">
                                    <a href="{{route('admin.tourist.articles.edit',$value->slug)}}" type="button" class="btn btn-success" style="border-radius: inherit">Sửa</a>
...
@section('script')
    <script>
        ...
        $(".btn-delete").click(function () {
            let value = $(this).val();
            let url = window.location.origin +'/admin/tourist/articles/delete/' + value;
            $.confirm({
                title: 'Xác nhận!',
                content: 'Bạn có chắc xóa bài viết này?',
                buttons: {
                    info: {
                        btnClass: 'btn-blue',
                        text: 'Xác nhận',
        ...
        $('input[name="display"]').change(function () {
            $.ajax({
                url: window.location.origin + '/admin/travel/articles/display',
                dataType: 'json',
                type: 'post',
                data: {'id' : $(this).val()},
                success: function (data) {
