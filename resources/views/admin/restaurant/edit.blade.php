        ...
        <section class="section dashboard">
            <div class="bg-white p-4">
                <div class="bg-white p-4">
                    ...
                    @endif
                    <form action="{{url('admin/restaurant/update/'.$restaurant->id)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-3">{{'Tên nhà hàng :'}}</div>
                            <div class="col-8">
                                <input class="form-control" name="name" value="{{$restaurant->name}}" type="text" required>
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-3">{{'Mô tả ngắn :'}}</div>
                            <div class="col-8">
                                <textarea class="form-control" name="content" type="text" maxlength="255" rows="4">{{$restaurant->content}}</textarea>
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-3">{{'Địa chỉ :'}}</div>
                            <div class="col-8">
                                <input class="form-control" name="address" value="{{$restaurant->address}}" type="text">
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-8">
                                <input class="form-control" name="rating" value="{{$restaurant->rating}}" type="number" max="5">
                            </div>
                        </div>
                        <div class="row mt-3">
                                ...
                                <div class="form-control position-relative div-parent" style="padding-top: 50%">
                                    <div class="position-absolute w-100 h-100 div-file" style="top: 0; left: 0;z-index: 10">
                                        ...
                                        @if($restaurant->type == 1)
                                            <img src="{{$restaurant->banner}}" class="w-100 h-100" style="object-fit: cover">
                                            @else
                                            <video class="w-100 h-100" style="object-fit: cover" controls>
                                                <source src="{{$restaurant->banner}}">
                                            </video>
                                        @endif
                                    </div>
                                </div>
