...
@section('main')
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1 class="text-capitalize">Sửa tìm kiếm dịch vụ theo vị trí</h1>
        </div><!-- End Page Title -->
        ...
        <section class="section dashboard">
            <div class="bg-white p-4">
                <div class="bg-white p-4">
                    ...
                    @endif
                    <form action="{{route('admin.introduce.searchServices.update',$SearchService->id)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row mt-3">
                            <div class="col-3">{{'Title :'}}</div>
                            <div class="col-8">
                                <input class="form-control" name="title" type="text" rows="4" value="{{$SearchService->title}}" />
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-3">{{'Nội dung :'}}</div>
                            <div class="col-8">
                                <textarea class="form-control" name="content" type="text" rows="4" >{{$SearchService->content}}</textarea>
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-3">{{'Đánh giá :'}}</div>
                            <div class="col-8">
                                <input class="form-control" name="ratings" type="number" value="{{$SearchService->ratings}}">
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-3">{{'Địa điểm :'}}</div>
                            <div class="col-8">
                                <input class="form-control" name="location" type="text" value="{{$SearchService->location}}">
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-3">{{'Thứ tự :'}}</div>
                            <div class="col-8">
                                <input class="form-control" name="index" type="number" value="{{$SearchService->index}}">
                            </div>
                        </div>
                        <div class="row mt-3">
                            ...
                            <div class="col-8">
                                <label class="switch">
                                    <input type="checkbox" @if($SearchService->display == 1) checked @endif name="display">
                            ...
                            <div class="col-8">
                                <div class="form-control position-relative div-parent" style="padding-top: calc(50% + 46px);">
                                    <button type="button" class="position-absolute border-0 bg-transparent select-image" style="top: 50%;left: 50%;transform: translate(-50%,-50%)">
                                        <img src="{{$SearchService->src}}" class="w-100 h-100" style="object-fit: cover">
                                    </button>
                                </div>
                            </div>
                        </div>
                        <div class="row mt-5">
                            ...
                            <div class="col-8 d-flex justify-content-end">
                                <a href="{{route('admin.introduce.searchServices.index')}}" class="btn btn-danger" style="margin-right: 20px">Hủy</a>
