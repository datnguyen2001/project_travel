<form action="{{route('web.touring.hotel',$hotel->id)}}" method="post" class="item-room position-relative">
    @csrf
    ...
    <div class="row border-bottom pb-3">
        <div class="col-4">
            ...
            @else
                <img src="{{$hotel->banner}}" class="w-100" style="border-radius: 4px;">
            @endif
        </div>
        <div class="col-8 d-flex align-items-center">
            <div>
                <p class="name-room">{{$hotel->name}}</p>
                <p class="price-room">{{number_format($hotel->price)}}đ - {{number_format($hotel->price_2)}}đ</p>
            </div>
        </div>
    </div>
    <div class="mt-2">
