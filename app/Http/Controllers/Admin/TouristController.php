...
class TouristController extends Controller
{
    ...
    {
        $listData =CategoryTouristModel::all();
        $titlePage = 'Địa điểm du lịch';
        $page_menu = 'tour';
        $page_sub = 'category';
        return view('admin.tour.category.index', compact('titlePage', 'page_menu', 'page_sub', 'listData'));
    }
    ...
    public function categoryCreate()
    {
        ...
        $page_menu = 'tour';
        $page_sub = 'category';
        return view('admin.tour.category.create', compact('titlePage', 'page_menu', 'page_sub'));
    }
    ...
    public function categoryStore (Request $request)
    {
        try{
            ...
            $category = new CategoryTouristModel([
            ...
            $category->save();
            $this->addMultimedia($request, $category->id, 8);
            return redirect()->route('admin.tourist.category.index')->with(['success' => 'Thêm danh mục thành công']);
    ...
    {
        try{
            $category = CategoryTouristModel::find($id);
            if (empty($category)){
                $data['status'] = false;
            ...
            $category->delete();
            $this->deleteMultimediaFiles($category->id, 8);
            $data['status'] = true;
    ...
    public function categoryEdit ($id)
    {
        ...
        $page_menu = 'tour';
        $page_sub = 'category';
        return view('admin.tour.category.edit', compact('titlePage', 'page_menu', 'page_sub', 'category', 'multimedia'));
    }
    ...
    public function categoryUpdate (Request $request, $id)
    {
        try{
            ...
            $category->save();
            $this->addMultimedia($request, $category->id, 8);
            return redirect()->route('admin.tourist.category.index')->with(['success' => 'Cập nhật danh mục thành công']);
        }catch (\Exception $exception){
            return back()->with(['error' => $exception->getMessage()]);
        }
    }
    ...
    {
        $listData =ArticlesReviewModel::where('category', 5)->orderBy('created_at', 'desc')->get();
        $titlePage = 'Địa điểm du lịch';
        $page_menu = 'tour';
        $page_sub = 'articles';
        return view('admin.tour.articles.index', compact('titlePage', 'page_menu', 'page_sub', 'listData'));
    }

    public function articlesCreate ()
    {
        $category = CategoryTouristModel::all();
        $titlePage = 'Địa điểm du lịch';
        $page_menu = 'tour';
        $page_sub = 'articles';
        return view('admin.tour.articles.create', compact('titlePage', 'page_sub', 'page_menu', 'category'));
    }
    ...
    public function articlesStore (Request $request)
    {
        try{
            ...
            $category = CategoryTouristModel::find($request->get('category'));
            ...
            $articles->save();
            $this->addMultimedia($request, $articles->id, 9);
            return redirect()->route('admin.tourist.articles.index')->with(['success' => 'Thêm bài viết mới thành công']);
        }catch (\Exception $exception){
            return back()->with(['error' => $exception->getMessage()]);
        }
    }
    ...
    {
        $articles = ArticlesReviewModel::where('category', 5)->where('slug', $slug)->first();
        if (empty($articles)){
            return back()->with(['error' => 'Bài viết không tồn tại']);
        }
        $category = CategoryTouristModel::all();
        $multimedia = MultimediaFilesModel::where('parent_id', $articles->id)->where('parent_type', 9)->get();
        $titlePage = 'Địa điểm du lịch';
        $page_menu = 'tour';
        $page_sub = 'articles';
        return view('admin.tour.articles.edit', compact('articles', 'category', 'titlePage', 'page_menu', 'page_sub', 'multimedia'));
    }

    public function articlesUpdate (Request $request, $id)
    {
        try{
            $articles = ArticlesReviewModel::where('category', 5)->where('id', $id)->first();
            if (empty($articles)){
                return back()->with(['error' => 'Bài viết không tồn tại']);
            }
            $slug = Str::slug($request->get('title'));
            ...
            $articles->display = isset($request->display) ? 1 : 0;
            $articles->save();
            $this->addMultimedia($request, $id, 9);
            return redirect()->route('admin.tourist.articles.index')->with(['success' => 'Sửa bài viết thành công']);
        }catch (\Exception $exception){
            return back()->with(['error' => $exception->getMessage()]);
        }
    }
    ...
    public function articlesDelete ($id)
    {
        try{
            ...
            $articles->delete();
            $this->deleteMultimediaFiles($id,9);
            $data['status'] = true;
